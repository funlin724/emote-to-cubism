...
- **没有基准预言机就没有调试方向**——先用官方运行时渲染截图做锚点
  对齐（以模型固定件求缩放平移，理想情况下比例应为 1.0000），再分区
  覆盖探查；对照必须覆盖全部可见特征（五官存在性、位置、透明度）再下
  "通过"结论——只查存在性会漏掉"鼻/嘴被去重吞掉"这类缺陷。
- **渲染正确 ≠ 数据完整**——平铺渲染对了，还要检查差分/状态数据是否
  保留（opa=0 图层是 Editor 绑定眨眼/表情/换装的原料）。提取器把
  "不可见件"一丢了之，渲染不坏，但下游可绑定性与可修正性全毁。

分层手段（按可信度排序）：

1. **渲染对照预言机**：官方运行时（如 FreeMoteViewer）打开解出的部件
   条目截图，与本管线产物做锚点对齐 + 分区 diff——换游戏的验收终审。
2. **pixi 帧缓冲预言机**（实验路线 moc3 用）：pixi-live2d-display 页内
   `gl.readPixels` 导出 PNG。页面截图会被视口裁切/合成干扰；桌面 Viewer
   的 JOGL 画布外部抓不到。
   - moc3 有**浏览器启发式缓存**——每次实验必须换目录或换 URL 参数。
   - 页面在后台时 rAF 冻结 → 保持面板可见，或用同步 `render()`。
3. **drawMesh 审计**：钩渲染器原型记录 (纹理号, 索引数, 顶点数, UV/位置, opacity)。
4. **软渲染对账**：numpy 逐三角形光栅化 moc3 文件，与 CPU 装配渲染逐像素比
   ——区分"文件错"还是"渲染管线错"的唯一手段。
5. **切割质检先行**（`tools/qc_parse.py`）：部件数三方一致 + 逐像素贴回
   比对。切割正确性先于装配语义被证明，把问题域砍掉一半——跨游戏移植
   时最值钱的一步。
6. **Editor 真值对照**：官方 Editor 导出的最小模型（1 网格+1 变形器+1 绑定）
   是绑定接线的权威参照——见 [editor-truth-recipe.md](editor-truth-recipe.md)。
7. **池清零对照 / 逐组隐藏 / 常量位置池 / MVP 捕获**：定位求值链路问题的
   四个关键判定实验。
8. ⚠ 核心包装层 getter（getDrawableVertexPositions 等）返回解析时拷贝，
